    ...
    const finalizeSubmission = async () => {
        ...
        try {
            ...
            await billingApi.recordPayment({
                amount: ((formData.dailyBudget || 20) * formData.durationDays).toFixed(2),
                            ...
                            <button
                                onClick={() => {
                                    setSuccess(null); setIsEditMode(false); setFormData({
                                        ...
                                        posterUrl: null, durationDays: 7, dailyBudget: 20, localAudioUrl: null
                                    });
                                }}
    ...
    return (
        <div className="max-w-4xl mx-auto py-10 px-4">
                ...
                <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-8">
                    ...
                    <div className="p-8 rounded-[40px] bg-white/5 border border-white/10 space-y-6 relative overflow-hidden group">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-white/5">
                            <div>
                                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-3 ml-2">Daily Budget ($)</label>
                                <input
                                    type="number"
                                    min="5"
                                    value={formData.dailyBudget || 20}
                                    onChange={(e) => setFormData(prev => ({ ...prev, dailyBudget: parseInt(e.target.value) || 0 }))}
                                    className="w-full bg-black/40 border-2 border-white/5 rounded-2xl px-6 py-4 focus:outline-none focus:border-tiktok-pink/50 transition-all font-bold"
                                />
                            </div>
                            <div>
                                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-3 ml-2">Duration (Days)</label>
                                <div className="flex items-center gap-4">
                                    <input
                                        ...
                                        className="flex-1 accent-tiktok-pink"
                                    />
                                    <span className="text-xl font-black italic text-white w-12 text-center">{formData.durationDays}d</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : isEditMode ? 'SAVE CHANGES' : 'REVIEW & PAY'}
                    </button>
                </form>
            ...
            <PaymentModal
                ...
                onPaymentSuccess={finalizeSubmission}
                amount={((formData.dailyBudget || 20) * formData.durationDays).toFixed(2)}
                campaignName={formData.campaignName}
            />
        </div>

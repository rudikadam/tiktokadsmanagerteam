...
const AdCreation = ({ user, setError }) => {
    ...
    const [paymentStep, setPaymentStep] = useState('INIT'); // INIT, QR, VERIFYING
    ...
    const [formData, setFormData] = useState({
        ...
        durationDays: 7,
        localAudioUrl: null,
        paymentMode: 'PAY_PER_CLICK', // PAY_PER_CLICK, PAY_PER_IMPRESSION
        dailyBudget: '50'
    });
    ...
    const validateField = (name, value) => {
        ...
        if (name === 'dailyBudget') {
            if (!value) error = 'Daily budget is required';
            else if (parseFloat(value) < 20) error = 'Minimum recommended budget is $20.00';
        }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const fieldNames = ['campaignName', 'adText', 'cta', 'dailyBudget'];
        ...
        try {
            ...
            // Artificial delay to show QR Code
            await new Promise(r => setTimeout(r, 4000));
            setPaymentStep('VERIFYING');
            await new Promise(r => setTimeout(r, 2000));

            const savedAds = JSON.parse(localStorage.getItem(storageKey) || '[]');
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-[32px] p-8 space-y-6 relative overflow-hidden group/payment">
                                <div className="absolute top-0 right-0 p-8 opacity-5 group-hover/payment:opacity-10 transition-opacity pointer-events-none">
                                    <ShieldCheck className="w-24 h-24 text-green-500" />
                                </div>
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-tiktok-cyan/10 rounded-2xl flex items-center justify-center border border-tiktok-cyan/20 text-tiktok-cyan shadow-lg shadow-tiktok-cyan/5">
                                            <CreditCard className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-0.5">Payment Mode</p>
                                            <p className="font-black text-white text-lg tracking-tight italic">
                                                {success.paymentMode === 'PAY_PER_IMPRESSION' ? 'CPM' : 'PPC'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2.5 px-4 py-2 bg-green-500/10 border border-green-500/20 rounded-2xl self-start sm:self-center">
                                        <ShieldCheck className="w-4 h-4 text-green-500" />
                                        <span className="text-[11px] font-black text-green-500 uppercase italic tracking-widest">Payment Done</span>
                                    </div>
                                </div>
                                <div className="h-px bg-white/10" />
                                <div className="flex items-center justify-between text-xs font-black tracking-[0.1em]">
                                    <span className="text-gray-500 uppercase">Transaction Status</span>
                                    <span className="text-tiktok-cyan animate-pulse italic">SUCCESSFUL</span>
                                </div>
                            </div>
                        </div>
            ...
            <div className="max-w-4xl mx-auto py-10 px-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-8">
                        ...
                        <div className="p-8 rounded-[40px] bg-white/5 border border-white/10 space-y-8 relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
                            ...
                            <div className="flex items-center gap-3 ml-2">
                                ...
                                <h3 className="text-sm font-black italic tracking-widest text-white uppercase">Payment & Budget</h3>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-3 ml-2 text-left">Daily Budget ($)</label>
                                    <div className="relative">
                                        <div className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-tiktok-cyan">$</div>
                                        <input
                                            name="dailyBudget"
                                            type="number"
                                            value={formData.dailyBudget}
                                            onChange={handleInputChange}
                                            className="w-full bg-black/40 border-2 border-white/5 rounded-2xl pl-10 pr-6 py-4 focus:outline-none focus:border-tiktok-cyan/50 transition-all font-black text-white"
                                            placeholder="50"
                                        />
                                    </div>
                                    <p className="text-[9px] text-gray-600 font-bold mt-2 ml-2 uppercase tracking-tighter italic">*Recommended min budget: $20.00</p>
                                </div>

                                <div className="space-y-3">
                                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-2 text-left">Billing Mode</label>
                                    <div className="grid grid-cols-1 gap-2">
                                        {[
                                            { id: 'PAY_PER_CLICK', label: 'PAY-PER-CLICK (PPC)', desc: 'Optimized for traffic' },
                                            { id: 'PAY_PER_IMPRESSION', label: 'PAY-PER-1K (CPM)', desc: 'Optimized for reach' }
                                        ].map(mode => (
                                            <button
                                                key={mode.id}
                                                type="button"
                                                onClick={() => setFormData(prev => ({ ...prev, paymentMode: mode.id }))}
                                                className={`p-4 rounded-2xl border-2 transition-all text-left flex items-center justify-between group ${formData.paymentMode === mode.id
                                                    ? 'bg-tiktok-cyan/10 border-tiktok-cyan'
                                                    : 'bg-white/3 border-white/5 hover:bg-white/5'
                                                    }`}
                                            >
                                                <div>
                                                    <p className={`text-[10px] font-black tracking-widest ${formData.paymentMode === mode.id ? 'text-tiktok-cyan' : 'text-white'}`}>{mode.label}</p>
                                                    <p className="text-[8px] text-gray-600 font-bold uppercase tracking-widest mt-0.5">{mode.desc}</p>
                                                </div>
                                                {formData.paymentMode === mode.id && <CheckCircle2 className="w-4 h-4 text-tiktok-cyan" />}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="p-6 bg-yellow-400/5 rounded-[32px] border border-yellow-400/10 flex items-start gap-4">
                                <Info className="w-5 h-5 text-yellow-500 flex-shrink-0 mt-0.5" />
                                <p className="text-[10px] text-gray-400 font-bold leading-relaxed uppercase tracking-widest text-left">
                                    Your card ending in <span className="text-white italic">4432</span> will be charged automatically when the budget reaches 80% threshold. You can manage payment methods in <span className="text-tiktok-cyan cursor-pointer hover:underline" onClick={() => navigate('/settings')}>Settings</span>.
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="space-y-8">
                        <div className="p-8 rounded-[40px] bg-white/5 border border-white/10 sticky top-24 shadow-3xl">
                            ...
                            <div className="aspect-[9/16] bg-black rounded-[40px] border-8 border-zinc-900 shadow-2xl relative overflow-hidden flex items-center justify-center group/preview">
                                ...
                                {/* Viral Benchmark Overlay */}
                                <div className="absolute top-24 left-6 z-20">
                                    <div className="flex flex-col gap-1">
                                        <div className="text-[8px] font-black text-white/40 uppercase tracking-widest mb-1">Viral Potential</div>
                                        <div className="flex gap-1">
                                            {[1, 2, 3, 4, 5].map((idx) => {
                                                const pot = (formData.adText.length > 20 ? 3 : 1) + (formData.musicOption !== 'none' ? 2 : 0);
                                                return (
                                                    <div
                                                        key={idx}
                                                        className={`w-4 h-1 rounded-full transition-all duration-500 ${idx <= pot ? 'bg-tiktok-pink shadow-[0_0_8px_rgba(254,44,85,0.5)]' : 'bg-white/10'}`}
                                                    />
                                                );
                                            })}
                                        </div>
                        ...
                        <div className="p-8 rounded-[40px] bg-white/5 border border-white/10 space-y-4">
                            <div className="flex items-center justify-between">
                                <h4 className="text-[10px] font-black tracking-widest uppercase text-gray-500">Viral Benchmark Status</h4>
                                <div className="flex items-center gap-1.5">
                                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                                    <span className="text-[10px] font-black text-green-500 uppercase italic">Calibration Active</span>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="flex-1">
                                    <p className="text-xl font-black italic tracking-tighter">OPTIMAL</p>
                                    <p className="text-[8px] text-gray-600 font-bold uppercase tracking-widest mt-1">Creative matches top 5% trends</p>
                                </div>
                                <Sparkles className="w-8 h-8 text-tiktok-pink animate-spin-slow" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <AnimatePresence>
                {isProcessingPayment && (
                    ...
                    >
                        ...
                        >
                            <div className="absolute -top-20 -left-20 w-40 h-40 bg-tiktok-cyan/10 blur-[60px] rounded-full" />

                            {paymentStep === 'QR' ? (
                                <>
                                    <div className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center text-tiktok-cyan mb-8 mx-auto border border-white/10">
                                        <Wallet className="w-10 h-10" />
                                    </div>
                                    <h3 className="text-3xl font-black mb-2 tracking-tighter">SECURE PAYMENT</h3>
                                    <p className="text-gray-500 text-sm mb-8 leading-relaxed font-medium">
                                        Scan the QR Code to complete {formData.campaignName} deployment
                                    </p>

                                    <div className="relative p-6 bg-white rounded-[32px] mx-auto mb-8 shadow-2xl">
                                        <QrCode className="w-48 h-48 text-black" />
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <div className="w-12 h-12 bg-white rounded-xl shadow-lg flex items-center justify-center p-1 border-4 border-white">
                                                <div className="w-full h-full bg-tiktok-pink rounded-md flex items-center justify-center">
                                    ...
                                    <div className="flex items-center gap-3 justify-center mb-6">
                                        <div className="w-1.5 h-1.5 rounded-full bg-tiktok-cyan animate-pulse [animation-delay:0s]" />
                                        <div className="w-1.5 h-1.5 rounded-full bg-tiktok-cyan animate-pulse [animation-delay:200ms]" />
                                        <div className="w-1.5 h-1.5 rounded-full bg-tiktok-cyan animate-pulse [animation-delay:400ms]" />
                                        <span className="text-[10px] font-black tracking-[0.2em] text-tiktok-cyan uppercase ml-1">Waiting for scan</span>
                                    </div>
                                    <div className="flex items-center justify-center gap-2 text-gray-600">
                                        <span className="text-[8px] font-black uppercase tracking-widest whitespace-nowrap">Gateway: {formData.paymentMode}</span>
                                    </div>
                                </>
                            ) : (
                                <div className="py-12 flex flex-col items-center">
                                    <div className="relative mb-8">
                                        <div className="w-20 h-20 border-4 border-tiktok-cyan/20 border-t-tiktok-cyan rounded-full animate-spin" />
                                        <ShieldCheck className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 text-tiktok-cyan" />
                                    </div>
                                    <h3 className="text-2xl font-black mb-2 tracking-tighter">VERIFYING TRANSACTION</h3>
                                    <p className="text-gray-500 text-sm italic font-medium">Validating blockchain receipt...</p>
                                </div>
                            )}
